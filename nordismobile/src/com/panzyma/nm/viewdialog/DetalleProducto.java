...
public class DetalleProducto extends Dialog  implements Handler.Callback {
    ...
    public void setOnDialogDetalleProductButtonClickListener(OnButtonClickHandler listener) {
		mButtonClickListener = listener;
	} 
	
    public DetalleProducto(DialogProducto dp,Producto prod, long idCategCliente, long idTipoPrecio, long idTipoCliente, boolean exonerado) {    
    	super(dp.getContext(),android.R.style.Theme_Translucent_NoTitleBar_Fullscreen);
...
//	
	...
	{
		//Via precio se habilita si codigo de tipo de cliente al que se está facturando es igual 
		...
		tboxproducto.setText(parent.getProductoSelected().getNombre());
        ...
        btnaceptar.setOnClickListener(new android.view.View.OnClickListener(){
			...
			public void onClick(View v) {
				...
				if(isValidInformation())
				{
					...
					{						
						...
						if (nuevaCantidadOrdenada != _det.getCantidadOrdenada()) 
						{
				           ...
				           {               
				               ...
				               //Validar que la edición de la cantidad bonificada sea válida
				               ...
				               if (!mcontext.getSharedPreferences("LoginUser", android.content.Context.MODE_PRIVATE).getBoolean("isPuedeEditarBonifAbajo",false)) {
				                    ...
				                    if ((chkViaPrecio == null) || (!chkViaPrecio.isChecked())) {
				                        Bonificacion b = getBonificacion(_producto, _idCategCliente, nuevaCantidadOrdenada);
				                        if (b != null) bonifProd = b.getCantBonificacion();
				                    }                   
				   ...
				   _det.setBonifEditada(false);
				   _det.setPrecioEditado(false);
				   if (_det.getCantidadBonificada() != _det.getCantidadBonificadaEditada()) _det.setBonifEditada(true);
				   if (_det.getPrecio() != _det.getMontoPrecioEditado()) _det.setPrecioEditado(true);
    ...
    public static ArrayList<Bonificacion> parseListaBonificaciones(Producto prod, long idCatCliente) {
        if (prod.getListaBonificaciones() == null) return null;
        if (prod.getListaBonificaciones() == "") return null;
    ...
    /*
    * Devuelve la bonificación de un producto dada la cantidad que se está ordenando y la categoría de cliente
    */
    public static Bonificacion getBonificacion(Producto prod, long idCatCliente, int cantidad) {
    	ArrayList<Bonificacion> bonificaciones = parseListaBonificaciones(prod, idCatCliente);
        if (bonificaciones == null) return null;
        ...
        //Recordar traer valor de parámetro DecimalRedondeoBonif (por el momento se usa 0.8)
        if (bb != null) {
    ...
    public static ArrayList<PrecioProducto> parseListaPrecios(Producto prod, long idTipoPrecio) {
        ...
        for(int i = 0; i < listaPrecios.length; i++) {
            ...
            if ((idTipoPrecio == 0) || (tipoPrecio.compareTo(precioProd[0]) == 0)) {
    ...
    */
	...
	{ 		
        if (tboxproducto.getText().toString().trim().length()==0){ 
        		tboxproducto.setError( "Debe haber primero seleccionado un producto.");
        		tboxproducto.requestFocus();
        		return false;
        }
        else if (tboxcantidad.getText().toString().trim().length()==0){ 
        		tboxcantidad.setError("La Cantidad a comprar debe ser mayor que cero");
