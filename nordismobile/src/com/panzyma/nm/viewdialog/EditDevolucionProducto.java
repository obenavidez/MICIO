...
public class EditDevolucionProducto extends DialogFragment {
	...
	private static final String TAG = EditDevolucionProducto.class.getSimpleName();
	...
	public class LoadDataToUI extends AsyncTask<Void, Void, List<Lote> > {
		...
		protected List<Lote> doInBackground(Void... params) {			
			return ModelLogic.getLotesByProductoID(getNMApp().getBaseContext(), product_selected.getId());
		}
	...
	private void initComponents() {
		...
		productName.setLayoutParams(
			    new TableRow.LayoutParams(
			        LayoutParams.WRAP_CONTENT, 
			        LayoutParams.MATCH_PARENT));				
		
		cmbLote.setLayoutParams(
			    new TableRow.LayoutParams(
			        LayoutParams.WRAP_CONTENT, 
			        LayoutParams.MATCH_PARENT));
				
		cantidad.setLayoutParams(
			    new TableRow.LayoutParams(
			        LayoutParams.WRAP_CONTENT, 
			        LayoutParams.MATCH_PARENT));
		
		tblRowVencimiento = (TableRow) view.findViewById(R.id.tblRowVencimiento);
			...
			cmbLote.setOnItemSelectedListener(new OnItemSelectedListener() {
				...
				public void onItemSelected(AdapterView<?> parentView,
					...
					if (position == (lotes.size()-1)){
						Lote _formaPago = (Lote) lotesAdapter.getItem(position).getObj();
	...
	private void establecer(List<Lote> ltes) {	
			...
			lotesAdapter = new CustomAdapter(getActivity(),
					R.layout.spinner_rows, setListData(lotes));
	...
	private ArrayList<SpinnerModel> setListData(List<Lote> lotes) {
		...
		for (Lote valor : lotes) {
			...
			sched.setDescripcion(String.valueOf(valor.getFechaVencimiento()).equals("0")?"":String.valueOf(valor.getFechaVencimiento()));			
			sched.setObj(valor);
