...
public class ModelRecibo {
	...
	public static void updateNotasCredito(List<ReciboDetNC> notasCredito, ContentResolver content,Context context){
		for(ReciboDetNC notaCredito : notasCredito){
			...
			// Filtro de factura
			String mWhereClause = NMConfig.Cliente.CCNotaCredito.Id +  " = " + notaCredito.getObjNotaCreditoID();				
			//Columnas a actualizar
			ContentValues mUpdateValues = new ContentValues();
...
//			mUpdateValues.put(NMConfig.Cliente.CCNotaCredito.Monto, abonado);
	...
	public static void updateNotasDebito(List<ReciboDetND> notasDebito, ContentResolver content,Context context){
		for(ReciboDetND notaDebito : notasDebito){
			...
			SQLiteDatabase db = DatabaseProvider.Helper.getDatabase(context);
			//Actualizar Factura
			db.update(DatabaseProvider.TABLA_CCNOTADEBITO, 
					mUpdateValues,
					mWhereClause,
					null);			
	...
	public synchronized static ArrayList<ReciboColector> getRecibosEnEstadoRegistrado(ContentResolver content)
	{
		...
		try {
				Cursor cur = content.query(DatabaseProvider.CONTENT_URI_RECIBO,
							projection, // Columnas a devolver
							"codEstado="+"'REGISTRADO'", // Condición de la query
							null, // Argumentos variables de la query
							null);
			    ReciboColector recibo = null;
			    if(cur.moveToFirst()){
